...
from toolbox.models.manage_dataset.utils import cif_to_pdbs, compress_and_save_h5, mkdir_for_batches
from toolbox.models.utils.cif2pdb import cif_to_pdb

def extract_archive(input_path, structures_dataset: "StructureDataset") -> Optional[Path]:
    ...
    if is_archive(input_path):
        ...
        if zipfile.is_zipfile(input_path):
            ...
            with tarfile.open(input_path, 'r:*') as tar_ref:  # 'r:*' auto-detects compression
...
def save_extracted_files(structures_dataset: "StructuresDataset", extracted_path, ids: Optional[List[str]]=None):
    ...
    def run(input_data, machine):
        return structures_dataset._client.submit(retrieve_protein_file_to_h5, *input_data, [machine], workers=[machine])
    ...
    compute_batches = ComputeBatches(structures_dataset._client, run, collect, "pdb_extracted_from_archive")
...
def retrieve_protein_file_to_h5(
    ...
    with worker_client() as client:
        ...
        converted_pdb_futures = client.map(file_to_pdb, pdb_futures,
                                           workers=workers)
...
def aggregate_results(protein_pdbs_with_cif: List[Dict[str, str]],
