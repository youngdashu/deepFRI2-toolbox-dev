...
def compress_and_save_h5_individual(
    ...
    with h5py.File(pdbs_file, "w") as hf:
        for pdb_name, pdb_content in zip(all_res_pdbs, all_contents):
            hf.create_dataset(
                pdb_name,
                data=np.frombuffer(pdb_content.encode("utf-8"), dtype=np.uint8),
                compression="gzip",
            )
    end_time = time.time()
...
def compress_and_save_h5_individual_lzf_shuffle(
    path_for_batch: Path, results: Tuple[List[str], List[str], List[str]]
):
    start_time = time.time()
...
def compress_and_save_h5_combined_lzf(
    ...
    with h5py.File(pdbs_file, "w") as hf:
        ...
        hf.create_dataset(
            ";".join(all_res_pdbs), data=compressed_content, compression="lzf"
        )
    end_time = time.time()
    total_time = end_time - start_time
    print(f"Compress time (combined): {total_time}")
...
def compress_and_save_h5_combined_lzf_shuffle(
    path_for_batch: Path, results: Tuple[List[str], List[str], List[str]]
):
    start_time = time.time()
