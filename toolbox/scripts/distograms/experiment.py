...
def process_file(file_path, start_index, keys_to_get):
    ...
    return [(key, create_single_data(hist_data, start_index + i), hist_data['data'], hist_data['max_distance'])
...
def generate_histogram(inputs):
    ...
    ax.set_title(f'3D Visualization of {len(inputs)} Distograms (Sorted by Max Distance)')
...
def generate_boxplot(inputs):
    ...
    ax.boxplot(binned_data, positions=bin_edges[:-1], widths=(bin_edges[1] - bin_edges[0]) * 0.8)
    ax.set_xlabel('Distance')
    ax.set_ylabel('Distribution')
    ax.set_title(f'Boxplot Visualization of Distances (Range: {bin_edges[0]:.2f} to {bin_edges[-1]:.2f})')
