...
def generate_schema(json_data: dict):
    gen_schema_proc = subprocess.Popen(
        ["genson"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, text=True
    )
...
def organise_json(json_data: dict):
    jq_proc = subprocess.Popen(
        ["jq", "."], stdin=subprocess.PIPE, stdout=subprocess.PIPE, text=True
    )
