...
def parse_html_to_json(html_file_path: str, parser="html.parser"):
    ...
    print(duration_minutes, duration_seconds)
...
def plot_scalability(df, plot_type="average"):
    ...
    if plot_type in ["average", "total"]:
        ...
        plt.title(
            f"Task Scalability ({plot_type.capitalize()} Time) with Total Duration"
        )

    elif plot_type == "duration":
        duration_df = (
            df.groupby("cores")["total_duration_minutes"].first().reset_index()
        )
        if any(duration_df["total_duration_minutes"] < 1):
            duration_df["total_duration_seconds"] = (
                df.groupby("cores")["total_duration_seconds"]
                .first()
                .reset_index()["total_duration_seconds"]
            )
            plt.plot(
                duration_df["cores"],
                duration_df["total_duration_seconds"],
            ...
            for _, row in duration_df.iterrows():
                plt.annotate(
                    f"{row['total_duration_seconds']:.1f}s",
            ...
            plt.plot(
                duration_df["cores"],
                duration_df["total_duration_minutes"],
                marker="o",
                color="blue",
            )
            ...
            for _, row in duration_df.iterrows():
                plt.annotate(
                    f"{row['total_duration_minutes']:.1f}m",
...
def plot_speedup(df):
    ...
    T1 = duration_df.loc[
        duration_df["cores"] == BASE_CORE_COUNT, "total_duration_seconds"
    ].values[0]
...
def plot_efficiency(df):
    ...
    duration_df["speedup"] = T1 / duration_df["total_duration_seconds"]
    duration_df["efficiency"] = (T1 / duration_df["total_duration_seconds"]) / (
        duration_df["cores"] / P1
    )
    ...
    for x, y in zip(duration_df["cores"], duration_df["efficiency"]):
        plt.text(
            x, y * 100 + 2, f"{y * 100:.1f}%", ha="center", va="bottom", fontsize=9
        )
...
def plot_grid(df, plot_dir="."):
    ...
    axs[1, 1].set_title(
        "Average Time vs Number of Cores (All Tasks)", fontsize=EXTRA_LARGE_FONT
    )
