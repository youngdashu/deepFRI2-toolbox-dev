...
def csv_to_latex(csv_file):
    ...
    avg_binary_retrieve = statistics.mean(float(row['Binary Retrieve (s)']) for row in rows)
    avg_text_convert = statistics.mean(float(row['Text Convert (s)']) for row in rows)
    avg_binary_convert = statistics.mean(float(row['Binary Convert (s)']) for row in rows)
    ...
    for row in rows:
        latex_output.write(f"{row['CPU cores']} & {float(row['Text Retrieve (s)']):.2f} & {float(row['Binary Retrieve (s)']):.2f} & ")
        latex_output.write(f"{float(row['Text Convert (s)']):.3f} & {float(row['Binary Convert (s)']):.3f} & ")
        latex_output.write(f"{float(row['Retrieve Diff (%)']):.2f} & {float(row['Convert Diff (%)']):.2f} \\\\\n")

    latex_output.write("\\hline\n")
    latex_output.write(f"Average & {avg_text_retrieve:.2f} & {avg_binary_retrieve:.2f} & ")
    ...
    for row in rows:
        latex_output.write(f"{row['CPU cores']} & {float(row['Text Total (s)']):.0f} & {float(row['Binary Total (s)']):.0f} & ")
